...
def cost(node, goal, heuristic='manhattan'):
    """
    Wrapper for multiple heuristic functions.
    """
    if heuristic == 'manhattan':
        return manhattan_heuristic(node, goal)
    elif heuristic == 'chebyshev':
        return chebyshev_heuristic(node, goal)
    elif heuristic == 'hybrid':
        return hybrid_heuristic(node, goal)
    else:
        raise ValueError("Invalid heuristic specified!")
...
def manhattan_heuristic(node, goal):
    """
    Manhattan heuristic: sum of absolute differences in x and y
    plus weighted orientation cost.
    """
    position_cost = abs(node[0] - goal[0]) + abs(node[1] - goal[1])
    angle_cost = min(abs(wrap_to_pi(node[2] - goal[2])), 2 * np.pi - abs(node[2] - goal[2]))
    return position_cost + 0.5 * angle_cost  # Weight orientation

def chebyshev_heuristic(node, goal):
    """
    Chebyshev heuristic: maximum of absolute differences in x and y
    plus weighted orientation cost.
    """
    position_cost = max(abs(node[0] - goal[0]), abs(node[1] - goal[1]))
    angle_cost = min(abs(wrap_to_pi(node[2] - goal[2])), 2 * np.pi - abs(node[2] - goal[2]))
    return position_cost + 0.5 * angle_cost  # Weight orientation

def hybrid_heuristic(node, goal, alpha=0.5):
    """
    Hybrid heuristic: weighted Manhattan and Chebyshev distance
    with orientation cost.
    """
    manhattan = abs(node[0] - goal[0]) + abs(node[1] - goal[1])
    chebyshev = max(abs(node[0] - goal[0]), abs(node[1] - goal[1]))
    angle_cost = min(abs(wrap_to_pi(node[2] - goal[2])), 2 * np.pi - abs(node[2] - goal[2]))
    return alpha * manhattan + (1 - alpha) * chebyshev + 0.5 * angle_cost
...
def main(screenshot=False):
    ...
    g_cost_manhattan, _, _, path_manhattan = a_star(start_config, goal_config, collision_fn, heuristic='manhattan')

    # Run with Chebyshev Heuristic
    g_cost_chebyshev, _, _, path_chebyshev = a_star(start_config, goal_config, collision_fn, heuristic='chebyshev')

    # Run with Hybrid Heuristic
    g_cost_hybrid, _, _, path_hybrid = a_star(start_config, goal_config, collision_fn, heuristic='hybrid')

    # Print Results
    print("Manhattan Heuristic: Path Cost =", g_cost_manhattan)
    print("Chebyshev Heuristic: Path Cost =", g_cost_chebyshev)
    print("Hybrid Heuristic: Path Cost =", g_cost_hybrid)
