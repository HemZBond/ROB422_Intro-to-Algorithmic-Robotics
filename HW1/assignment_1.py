...
def generate_edges(phi, n):
    ...
    array([[ 0.707107,  0.      ,  0.707107],
           ...
           [-0.353553, -0.612372,  0.707107]])
    """
    cone_edges = np.zeros((n, 3))
    for i in range(n):
        theta = 2 * np.pi * i / n
        x = np.cos(theta) * np.cos(phi)
        y = np.sin(theta) * np.cos(phi)
        z = np.sin(phi)
        cone_edges[i] = np.array([x, y, z])
...
def compute_normals(cone_edges):
    ...
    """
    n = len(cone_edges)
    facet_normals = np.zeros((n, 3))
    for i in range(n):
        facet_normals[i] = np.cross(cone_edges[i], cone_edges[(i+1) % n])
...
def compute_minimum_distance_from_facet_normals(a, facet_normals):
    ...
    True
    """
    distances = np.abs(facet_normals @ a) / np.linalg.norm(facet_normals, axis=1)
...
def check_is_interior_point(a, n, phi):
    ...
    is_interior_point = np.all(facet_normals @ a > 0)
    if np.all(facet_normals @ a >= 0):
